...
| **Asset Type** | IT Standard |
| **Control Record** | IT Standard Directory Entry |
...
| POST | `/v1/it-standard-directory-entry/initiate` | Initiate |
| GET | `/v1/it-standard-directory-entry` | Retrieve (list) |
| GET | `/v1/it-standard-directory-entry/{crId}/retrieve` | Retrieve |
| PUT | `/v1/it-standard-directory-entry/{crId}/update` | Update |
| PUT | `/v1/it-standard-directory-entry/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/it-standard-directory-entry/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t bian/sd-it-standards-and-guidelines:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install sd-it-standards-and-guidelines ./helm -n bian-business-support
```

Exposed through the platform Gateway at path prefix `/sd-it-standards-and-guidelines` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
